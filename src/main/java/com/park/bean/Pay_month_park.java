...
public class Pay_month_park implements Cloneable , Serializable{
    ...
    public Date ctime=new Date();//datetime    下单时间
    public Date utime=new Date();//datetime    更新支付状态时间
    ...
    public Date start_time=new Date();//datetime    启始时间
    public Date end_time=new Date();//datetime    到期时间
    ...
    public Date outtime_time=new Date();//datetime    超时结算时间（2016-08-1012：00：00）
    ...
    public Date outtime_paytime=new Date();//datetime    超时缴费时间
    ...
    public Date open_time=new Date();//datetime    开闸时间
    public int is_cash;//int(11)    是否现金支付0：在线支付1：现金支付
    public int park_type;//int(11)    停车场类型0：道闸停车场1：占道车场2：露天立体车库停车场
    public Date scan_time=new Date();//datetime    是否可以停车缴费时间（摄像头是否扫描到了：仅限道闸停车使用）
    public Date arrive_time=new Date();//datetime    该次入库的到达时间
    ...
    public Date stime=new Date();//datetime    服务器端接收数据创建时间
    ...
    public Date getOuttime_time(){
        return outtime_time;
    }

    public void setOuttime_time(Date value){
    	if(value == null){
           value = new Date();
        }
    ...
    public Date getOuttime_paytime(){
        return outtime_paytime;
    }

    public void setOuttime_paytime(Date value){
    	if(value == null){
           value = new Date();
        }
    ...
    public void setOpen_time(Date value){
    	if(value == null){
           value = new Date();
        }
    ...
    public void setScan_time(Date value){
    	if(value == null){
           value = new Date();
        }
    ...
    public void setArrive_time(Date value){
    	if(value == null){
           value = new Date();
        }
    ...
    public void setStime(Date value){
    	if(value == null){
           value = new Date();
        }
        this.stime= value;
    ...
    public static Pay_month_park newPay_month_park(long id, long pi_id, long ui_id, String car_code, int money, int pp_state, Date ctime, Date utime, int month_num, int pay_source, String my_order, String other_order, int pay_type, String pp_versioncode, int phone_type, int order_type, Date start_time, Date end_time, String month_info, int allege_state, String area_code, int is_del, long upc_id, long discount_money, String note, String discount_name, int discount_type, int final_time, String address_name, String base_rate, String outtime_rate, int outtime_money, Date outtime_time, int outtime_minute, int outtime_start_price, int outtime_start_minute, int outtime_charge_money, int outtime_charge_minute, String outtime_other_order, Date outtime_paytime, int outtime_paystate, String permit_time, int is_24hours, int is_open, Date open_time, int is_cash, int park_type, Date scan_time, Date arrive_time, long temp_money, String pi_name, int is_over, int cancel_state, long pu_id, String pu_nd, double lng, double lat, int is_expire, int rent_state, String ui_nd, long ai_id, int ai_money, int ai_effect, String ui_tel, int magnetic_state, String gov_num, Date stime, String up_orderid) {
    ...
    public void assignment(Pay_month_park pay_month_park) {
        ...
        Date ctime = pay_month_park.getCtime();
        Date utime = pay_month_park.getUtime();
        ...
        Date start_time = pay_month_park.getStart_time();
        Date end_time = pay_month_park.getEnd_time();
        ...
        Date outtime_time = pay_month_park.getOuttime_time();
        ...
        Date outtime_paytime = pay_month_park.getOuttime_paytime();
        ...
        Date open_time = pay_month_park.getOpen_time();
        int is_cash = pay_month_park.getIs_cash();
        int park_type = pay_month_park.getPark_type();
        Date scan_time = pay_month_park.getScan_time();
        Date arrive_time = pay_month_park.getArrive_time();
        ...
        Date stime = pay_month_park.getStime();
