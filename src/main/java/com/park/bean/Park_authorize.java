...
public class Park_authorize implements Cloneable , Serializable{
    ...
    public Date last_time=new Date();//datetime    上次授权成功时间
    ...
    public void setLast_time(Date value){
    	if(value == null){
           value = new Date();
        }
    ...
    public static Park_authorize newPark_authorize(long pa_id, long pi_id, String area_code, String pi_name, String secret_key, int state, int duration, Date last_time, int encrypt_type, int is_del, Date ctime, Date utime, String note) {
    ...
    public void assignment(Park_authorize park_authorize) {
        ...
        Date last_time = park_authorize.getLast_time();
        int encrypt_type = park_authorize.getEncrypt_type();
        int is_del = park_authorize.getIs_del();
        Date ctime = park_authorize.getCtime();
        Date utime = park_authorize.getUtime();
