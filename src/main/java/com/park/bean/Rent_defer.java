...
public class Rent_defer implements Cloneable , Serializable{
    ...
    public Date starttime=new Date();//datetime    开始时间
    public Date endtime=new Date();//datetime    到期时间
    public int month_num;//int(11)    续约月份个数
    public Date ctime=new Date();//datetime    客户端创建时间
    public Date stime=new Date();//datetime    服务器端接收时间
    ...
    public static Rent_defer newRent_defer(long rd_id, String rent_order_id, long pi_id, String area_code, String pi_name, int money, int unit_price, Date starttime, Date endtime, int month_num, Date ctime, Date stime, Date utime, long ui_id, String ui_nd, String car_code, long pu_id, String pu_nd, String permit_time, int pay_state, int defer_state, String up_orderid, int flag, int pay_source, int mq_state, int rent_type, int is_del, String note, String father_order_id, String son_order_id, int is_expire, String client_order_id, String client_rule_id, int allege_state, long cpr_id) {
    ...
    public void assignment(Rent_defer rent_defer) {
        ...
        Date starttime = rent_defer.getStarttime();
        Date endtime = rent_defer.getEndtime();
        int month_num = rent_defer.getMonth_num();
        Date ctime = rent_defer.getCtime();
        Date stime = rent_defer.getStime();
        Date utime = rent_defer.getUtime();
